...
public class TaskMagicDynamicRegistry extends AbstractMagicDynamicRegistry<TaskInfo> {
  ...
  public TaskMagicDynamicRegistry(
      MagicResourceStorage<TaskInfo> magicResourceStorage,
  ...
  @EventListener(condition = "#event.getType() == 'task'")
  ...
  protected boolean register(MappingNode<TaskInfo> mappingNode) {
    ...
    if (taskScheduler != null) {
      ...
      logger.debug(
          "注册定时任务:[{},{}]",
          MagicConfiguration.getMagicResourceService().getScriptName(entity),
          entity.getCron());
    }
